...
namespace Survival_DevelopFramework.Items.DramaManager
{
    class DramaMgr
    {
        ...
        /// 单例
        /// </summary>
        private static DramaMgr instance;
        public static DramaMgr Instance
        {
            get
            {
                if (instance != null)
        ...
        public void RenderGameRole()
        {
            ...
            if (roleLeft != null)
            {
                ...
                Painter.DrawT(roleLeft, new Vector2(xPos, yPos),color);
            }
            if (roleRight != null)
            {
                Color color;
                ...
                Painter.DrawT(roleRight, new Vector2(xPos, yPos), color);
            }
        }
        /// <summary>
        /// 绘制用户界面
        ...
        public void RenderGameUI()
        {
            ...
            if (showDialog)
            {
                ...
                Painter.DrawT(dramaUI, new Rectangle(0, yPos, DialogLeftGfxRect.Width, DialogLeftGfxRect.Height), DialogLeftGfxRect);
                Painter.DrawT(dramaUI, new Rectangle(DialogLeftGfxRect.Width, yPos, blandWidth, DialogMiddleGfxRect.Height), DialogMiddleGfxRect);
                Painter.DrawT(dramaUI, new Rectangle(DialogLeftGfxRect.Width + blandWidth, yPos, DialogRightGfxRect.Width, DialogRightGfxRect.Height), DialogRightGfxRect);
                // 绘制闪烁NextButton
                xPos = Survival_Game.Width - 67;
                yPos = Survival_Game.Height - 53;
                float alphaValue = 0;
                float t = BaseGame.TotalTimeMilliseconds % 2000;
                alphaValue = t < 1000 ? (t / 1000.0f) : 2 - t / 1000;
                Painter.DrawT(dramaUI, new Rectangle(xPos, yPos, 20, 20), NextButtonGfxRect, new Color(1, 1, 1, alphaValue));
            }
        }
        ...
        public void RenderShowPicture()
        {
            ...
            Painter.DrawT(showPicture, picRect);
        ...
        {
            drama = new Drama(DramaData.Load(dramaName));
            UpdateDrama();
        }
        #endregion
            ...
            TestGame.StartTest("Test Drama",
                ...
                delegate
                {
                   // GameMgr.Initialize();
                    ...
                    dmgr.Initialize();
                    dmgr.LoadContent();
                    dmgr.StartDrama("Dialog");
                },
                delegate
                {
                    if (InputKeyboards.isKeyJustPress(Keys.Space))
                    {
                        if (dmgr.UpdateDrama() == DramaState.Finished)
                        {
                            GameMgr.GameEnd();
                        }
                    }
